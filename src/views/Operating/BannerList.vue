    ...
    <div class="UserList">
        <div class="HeaderComponent">
            <div class="row">
                ...
                <div style="font-size: 17px;">轮播管理</div>
            </div>
            <div>
        ...
        <div class="content">
            <div>
                <Card dis-hover>
                    ...
                    <div class="search">
                        <div class="row">
                            <div class="name">轮播名称：</div>
                            <Input v-model="formInline.carouselName" size="small" placeholder="轮播名称" style="width: 150px"/>
                        </div>
                        <div class="row">
                            <div class="name">轮播类型：</div>
                            <Select v-model="formInline.carouselType" size="small" style="width:150px" placeholder="全部">
                        ...
                        <div class="row">
                            ...
                            <Select v-model="formInline.carouselStatus" size="small" style="width:150px" placeholder="全部">
                ...
                <Card dis-hover>
                    ...
                    <div class="table">
                        <Table border :columns="columns" size="small" :data="data" :loading="loading">
                            ...
                            <template slot-scope="{ row, index }" slot="carouselType">
                                {{row.carouselType == 1 ? '活动' : row.carouselType == 2 ? '公告' : '直播预告' }}
                            </template>
                            <template slot-scope="{ row, index }" slot="status">
                                {{row.carouselStatus == 0 ? '下架' : '上架' }}
                            </template>
...
<script>
    ...
    export default {
        ...
        methods: {
            formatTs: formatTs,
            onAdd () {
                this.$router.push({path: '/AddBannerList'})
            },
            onEdit (record) {
                this.$router.push({
                    path: '/AddBannerList',
            ...
            onReset () {
                this.formInline.carouselName = ''
                this.formInline.carouselStatus = ''
                this.formInline.carouselType = ''
            ...
            async LoadData () {
                try {
                    ...
                    formData.append('carouselName', this.formInline.carouselName);
                    formData.append('carouselStatus', this.formInline.carouselStatus);
                    formData.append('carouselType', this.formInline.carouselType);
                    formData.append('pageNum', this.formInline.pageNum);
                    formData.append('pageSize', this.formInline.pageSize);
                    let result = await api.BannerList(formData)
                    if (result.code === 200) {
